...
<main class="postsAndComments">
    <div class="posts">
        ...
        <p><?= wordwrap($post[0]->posttext, 150, "<br />\n"); ?></p>
        <p><a href="<?= $showAll . "/" . $userController->returnId($post[0]->postname) ?>"><?= $post[0]->postname ?></a></p>
        <?= $p ?>
    ...
    <div>
        <?php if ($this->di->get("session")->has("email")) : ?>
            <h4 class="createComment">Create <a href="<?= $createNewComment . "/" . $post[0]->postid ?>">New Comment</a>
            </h4>
        <?php else : ?>
    ...
    <div class="comments">
        <?php foreach ($comments as $con => $value) : ?>
            <div class="test">
                <h5><?= wordwrap($value[0]->commenttext, 200, "<br />\n"); ?></h5>

                <h6><a href="<?= $showAll . "/" . $userController->returnId($value[0]->Author) ?>"><?= $value[0]->Author ?></a></h6>
                <img src="<?php echo $gravatar->getGravatar($value[0]->Author, 40); ?>" alt=""/>
                <?php if ($currentUser === $value[0]->Author || $permissions === "admin") : ?>
                    <a href="<?= $deleteComment . "/" . $value[0]->idcomment ?>">Delete Comment</a>
                    <a href="<?= $editComment . "/" . $value[0]->idcomment ?>">Edit Comment</a>
                <?php endif ?>
                <a href="<?= $createNewCommentComment . "/" . $value[0]->idcomment . "/" . $post[0]->postid ?>">Add new Comment</a>
                    <?php foreach ($value[1] as $v) : ?>
                        <div class="commentcomments">
                            <h5><?= wordwrap($v->textcomment, 200, "<br />\n"); ?></h5>
                            <h6><a href="<?= $showAll . "/" . $userController->returnId($v->postuser) ?>"><?= $v->postuser ?></a></h6>
                            <img src="<?php echo $gravatar->getGravatar($v->postuser, 40); ?>" alt=""/>
                        </div>
                        <?php if ($currentUser === $v->postuser || $permissions === "admin") : ?>
                            <a href="<?= $deleteCommentComment . "/" . $v->idcommentc ?>">Delete Comment</a>
                            <a href="<?= $editCommentComment . "/" . $v->idcommentc ?>">Edit Comment</a>
                        <?php endif ?>
                    <?php endforeach; ?>
            </div>
        <?php endforeach; ?>
